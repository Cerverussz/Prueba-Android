...
abstract class CustomAdapter<T, VH : RecyclerView.ViewHolder> : RecyclerView.Adapter<VH>() {
    ...
    fun animateTo(models: ArrayList<T>) {
        applyAndAnimateRemovals(models)
        applyAndAnimateAdditions(models)
        applyAndAnimateMovedItems(models)
    }

    private fun applyAndAnimateRemovals(newModels: ArrayList<T>) {
    ...
    private fun applyAndAnimateAdditions(newModels: ArrayList<T>) {
        var i = 0
    ...
    private fun applyAndAnimateMovedItems(newModels: ArrayList<T>) {
